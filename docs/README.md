...
1. **Install dependencies**
   ...
   > **Note**: The default setup command downloads packages such as `numpy`
   > and `torch` from PyPI. Ensure the environment has outbound internet access
   > or a pre-populated wheel cache/proxy; otherwise installation will fail
   > before the training pipeline can run.
